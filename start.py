...
VOICES_DIR = ROOT / "voices"             # Piper TTS voices (<key>.onnx + <key>.onnx.json)
PREVIEWS_DIR = VOICES_DIR / "previews"   # generated voice preview samples (<key>.wav)
...
DEFAULT_SETTINGS = {
    ...
    "piperVoice": "",            # selected voice key (filename stem under voices/)
...
# ────────────────────────── Piper TTS engine ─────────────────────────
#
# Optional neural text-to-speech via the `piper-tts` PyPI package
# (`pip install piper-tts`). The terminal still runs without it — the engine
# is only touched when the UI's SPEECH section is set to the "piper" engine.
#
# Voices live in voices/ as a <key>.onnx model paired with a <key>.onnx.json
# (or <key>.json) config — the standard Piper layout. At most ONE voice is held
# in memory; switching voices unloads the previous model (del + gc.collect())
# so memory does not accumulate. The browser drives synthesis one text block at
# a time so playback can start as soon as the first block of a reply is ready.

PREVIEW_SAMPLE_TEXT = (
    "Section nine, online. This is a Piper text to speech voice preview."
)

def _safe_voice_key(k) -> bool:
    """Voice keys come from filenames — keep them path-traversal safe."""
    return (
        isinstance(k, str)
        and 0 < len(k) <= 160
        and ".." not in k
...
class TTSEngine:
    """Lazy, thread-safe wrapper around piper-tts. Robust across API revisions."""

    def __init__(self):
        self._lock = threading.RLock()
        self._voice = None          # currently loaded PiperVoice
        self._voice_key = None      # key of the loaded voice
        self._import_error = None   # last import failure (string), if any

    # -- import (deferred: piper pulls in onnxruntime, which is heavy) --
    def _piper(self):
        try:
    ...
    # -- voice discovery --
    @staticmethod
    def _config_for(onnx: Path):
        for cand in (Path(str(onnx) + ".json"), onnx.with_suffix(".json")):
            if cand.exists():
                return cand
        return None
    ...
    def list_voices(self) -> list[dict]:
        ...
        for onnx in sorted(VOICES_DIR.glob("*.onnx")):
            ...
            lang = ""
            try:
                ln = meta.get("language") or {}
                lang = ln.get("name_native") or ln.get("name_english") or ln.get("code") or ""
            except Exception:
                lang = ""
            quality = ""
            try:
                quality = (meta.get("audio") or {}).get("quality", "") or meta.get("quality", "")
            except Exception:
                quality = ""
            out.append({
                "key": key,
                "name": meta.get("dataset") or key,
                "language": lang,
                "quality": quality,
                "hasConfig": cfg is not None,
                "hasPreview": (PREVIEWS_DIR / f"{key}.wav").exists(),
            })
        return out

    # -- model lifecycle (one loaded at a time → bounded memory) --
    ...
    def _load_locked(self, key: str):
        ...
        PiperVoice = getattr(piper, "PiperVoice", None)
        if PiperVoice is None:
            raise RuntimeError("incompatible piper-tts: PiperVoice not found")
        onnx = VOICES_DIR / f"{key}.onnx"
        if not onnx.exists():
            raise FileNotFoundError(f"voice model not found: voices/{key}.onnx")
        cfg = self._config_for(onnx)
        self._unload_locked()   # free previous model BEFORE loading the next
        try:
            voice = (PiperVoice.load(str(onnx), config_path=str(cfg))
                     if cfg is not None else PiperVoice.load(str(onnx)))
        except TypeError:
    ...
    # -- synthesis: one text block → a complete WAV byte string --
    @staticmethod
    ...
    def _syn_config(self, piper, length_scale, noise_scale, noise_w):
        if length_scale is None and noise_scale is None and noise_w is None:
            return None
        SC = getattr(piper, "SynthesisConfig", None)
        if SC is None:
            return None
        kw = {}
        ...
        if noise_scale is not None:
            kw["noise_scale"] = float(noise_scale)
        if noise_w is not None:
            kw["noise_w"] = float(noise_w)
        try:
            return SC(**kw)
        except Exception:
            return None

    def synth_wav(self, text: str, key: str,
                  length_scale=None, noise_scale=None, noise_w=None) -> bytes:
        import io
        import wave
        text = (text or "").strip()
        if not text:
        ...
        with self._lock:
            piper = self._piper()
            voice = self._load_locked(key)
            sample_rate = 22050
            try:
                sample_rate = int(getattr(voice, "config").sample_rate)
            except Exception:
                pass

            # piper 1.3+ : synthesize_wav(text, wave_write, syn_config=None)
            if hasattr(voice, "synthesize_wav"):
                buf = io.BytesIO()
                syn = self._syn_config(piper, length_scale, noise_scale, noise_w)
                with wave.open(buf, "wb") as wf:
                    try:
                        if syn is not None:
            ...
            # piper 1.2 : synthesize_stream_raw(text, **kw) → raw 16-bit PCM
            if hasattr(voice, "synthesize_stream_raw"):
                ...
                if noise_w is not None:
                    kw["noise_w"] = float(noise_w)
                pcm = b"".join(voice.synthesize_stream_raw(text, **kw))
                return self._pcm_to_wav(pcm, sample_rate)

            # fallback : synthesize() yields AudioChunk objects
            if hasattr(voice, "synthesize"):
                ...
                return self._pcm_to_wav(b"".join(parts), sr)

            raise RuntimeError("incompatible piper-tts: no known synthesis method")

    # -- previews: synthesise a sample for every voice that lacks one --
    def ensure_previews(self, sample_text: str = PREVIEW_SAMPLE_TEXT) -> dict:
        voices = self.list_voices()
        if not self.installed():
            return {"generated": [], "failed": [], "voices": voices,
                    "error": "piper-tts is not installed — run: pip install piper-tts"}
        if not voices:
            return {"generated": [], "failed": [], "voices": voices,
                    "error": "no voices found in voices/"}
        PREVIEWS_DIR.mkdir(parents=True, exist_ok=True)
        generated, failed = [], []
        for v in voices:
            key = v["key"]
            out = PREVIEWS_DIR / f"{key}.wav"
            if out.exists():
                continue
            try:
                out.write_bytes(self.synth_wav(sample_text, key))
                generated.append(key)
            except Exception as e:
                failed.append({"key": key, "error": str(e)})
        self.unload()   # free the batch model when done
        return {"generated": generated, "failed": failed, "voices": self.list_voices()}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send_bytes(self, code: int, data: bytes, ct: str, extra: dict | None = None):
        self._hdr(code, ct, len(data), extra=extra)
        ...
        try:
            ...
            if path == "/api/tts/voices":
                return self._send_json(200, {
                    "installed": TTS.installed(),
                    "dir": str(VOICES_DIR),
                    "voices": TTS.list_voices(),
                })
            if path == "/api/tts/preview":
                qs = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
                return self._serve_preview((qs.get("voice") or [""])[0])
            if path == "/api/events":
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/tts/previews/generate":
                return self._send_json(200, TTS.ensure_previews())
            if path == "/api/tts/unload":
                TTS.unload()
                return self._send_json(200, {"ok": True})
    ...
    def _serve_preview(self, key: str):
        if not _safe_voice_key(key):
            return self._send_json(400, {"error": "invalid voice key"})
        f = PREVIEWS_DIR / f"{key}.wav"
        if not f.exists():
            return self._send_json(404, {"error": "no preview for this voice yet"})
        self._send_bytes(200, f.read_bytes(), "audio/wav", extra={"Cache-Control": "no-store"})

    def _tts_speak(self, body: dict):
        text = body.get("text") or ""
        key = body.get("voice") or ""
        if not _safe_voice_key(key):
            return self._send_json(400, {"error": "no Piper voice selected"})
        ls = body.get("lengthScale")
        ns = body.get("noiseScale")
        nw = body.get("noiseW")
        try:
            wav = TTS.synth_wav(text, key, length_scale=ls, noise_scale=ns, noise_w=nw)
            ...
            msg = str(e)
            code = 503 if "not installed" in msg or "incompatible" in msg else 500
            return self._send_json(code, {"error": msg})
        self._send_bytes(200, wav, "audio/wav", extra={"Cache-Control": "no-store"})
...
def main():
    ...
    try:
        PREVIEWS_DIR.mkdir(parents=True, exist_ok=True)  # also creates voices/
    except Exception:
        pass
    ...
    _piper_ok = TTS.installed()
    _nvoices = len(TTS.list_voices())
    print(f"   tts          {'piper-tts ✓' if _piper_ok else 'piper-tts not installed (blip SFX only)'}"
          f" · {_nvoices} voice(s) in {VOICES_DIR.name}/")
